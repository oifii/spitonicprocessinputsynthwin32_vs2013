...
#ifndef InputExpSynth_h
#define InputExpSynth_h
...
class InputExpSynth : public Synth {
...
public:
	...
	InputExpSynth(){
		...
		ControlParameter ringFreq = addParameter("ringFreq", 1.f).displayName("Ring Mod Frequency").min(10.f).max(1000.f).logarithmic(true);
		...
		Generator ringModInput = inputReader * (inputGain >> ControlDbToLinear()).smoothed() * SineWave().freq(ringFreq.smoothed(0.5)); //spi
		...
		//BasicDelay delay2 = BasicDelay(1.2f).delayTime(0.75f).wetLevel(0.4f).dryLevel(0.0f).feedback(0.3f); //spi

		//spi note: see ReverbTestSynth class for control delay usage

		setOutputGen((ringModInput >> delay) * 0.5f);
		//setOutputGen((delay.input(ringModInput)) * 0.5f + delay2.input(ringModInput));
		//setOutputGen(ringModInput);
	}

};

TONIC_REGISTER_SYNTH(InputExpSynth);
